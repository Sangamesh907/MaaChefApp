...
export default function OrdersScreen() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      let response;
      if (activeTab === "New") {
        response = await ChefService.getOrders("incoming");
      } else if (activeTab === "Ongoing") {
        response = await ChefService.getOrders("ongoing");
      } else {
        response = await ChefService.getOrders("completed");
      }
                ...
                <View>
                  ...
                  <Text style={styles.items}>
                    {order.items && order.items.length > 0
                      ? order.items.map((i) => `${i.food_name || i.name} x ${i.quantity} (₹${i.price})`).join(", ")
                      : "No items"}
                  </Text>
                </View>
                <Text style={styles.amount}>₹ {order.total || order.total_price || 0}</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  tabs: { flexDirection: "row", justifyContent: "space-around", borderBottomWidth: 1, borderColor: "#eee" },
  ...
  statusBtn: { borderWidth: 1, borderColor: "#008080", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20 },
  statusText: { color: "#008080", fontWeight: "500" },
  viewBtn: { backgroundColor: "#D8F2EC", paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20 },
